    ...
    <div className="space-y-8 max-w-3xl">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm">₦250,000 Cloud Native Mentorship — Payment Ledger</p>
        </div>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 relative overflow-hidden">
        ...
        <div className="grid grid-cols-3 gap-4 mb-8 relative z-10">
          {[
            ...
            <div key={label} className="bg-slate-950/60 border border-slate-800/60 rounded-2xl p-4 text-center">
              <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-600 mb-2">{label}</p>
        ...
        <div className="relative z-10">
          <div className="flex justify-between text-[10px] font-bold uppercase tracking-wider text-slate-600 mb-2">
          ...
          <div className="flex justify-between mt-3">
            {[
              ...
              <div key={label} className="flex flex-col items-center gap-1" style={{ marginLeft: pct === 40 ? "40%" : 0 }}>
                <div className={`w-1 h-2 rounded-full ${billing.total_paid >= (TOTAL_DUE_KOBO * pct) / 100 ? "bg-yellow-500" : "bg-slate-700"}`} />
          ...
          <div className="space-y-5">
            <div>
              ...
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 font-bold text-lg pointer-events-none">₦</span>
                <input
                  ...
                  className={`w-full bg-slate-950 border rounded-xl pl-10 pr-4 py-4 text-xl font-bold font-mono text-slate-100 focus:outline-none focus:ring-2 focus:ring-yellow-500/30 transition-all ${
                    amountError ? "border-red-500/50" : "border-slate-800 focus:border-yellow-500/40"
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-3xl p-8">
        <h3 className="text-lg font-bold mb-6 flex items-center gap-3">
          ...
          Transaction History
          <span className="ml-auto text-[10px] font-bold uppercase tracking-widest text-slate-600 bg-slate-800 px-2.5 py-1 rounded-full">
        ...
        {payment_history.length === 0 ? (
          <div className="text-center py-12 border border-dashed border-slate-800 rounded-2xl">
            <ReceiptText className="w-10 h-10 text-slate-700 mx-auto mb-3" />
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-800 bg-slate-950/60">
                  <th className="text-left px-5 py-3 text-[10px] font-bold uppercase tracking-widest text-slate-600">#</th>
                  <th className="text-left px-5 py-3 text-[10px] font-bold uppercase tracking-widest text-slate-600">Date</th>
                  <th className="text-left px-5 py-3 text-[10px] font-bold uppercase tracking-widest text-slate-600">Amount</th>
                  <th className="text-left px-5 py-3 text-[10px] font-bold uppercase tracking-widest text-slate-600">Gateway</th>
                  <th className="text-left px-5 py-3 text-[10px] font-bold uppercase tracking-widest text-slate-600">Reference</th>
                  <th className="text-left px-5 py-3 text-[10px] font-bold uppercase tracking-widest text-slate-600">Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/60">
                {payment_history.map((tx: PaymentHistoryItem, idx: number) => (
                  ...
                  >
                    <td className="px-5 py-4 text-slate-600 font-mono text-xs">{payment_history.length - idx}</td>
                    <td className="px-5 py-4 text-slate-400 text-xs whitespace-nowrap">{fmtDate(tx.created_at)}</td>
                    <td className="px-5 py-4">
                      <span className="font-bold font-mono text-emerald-400 text-sm">{fmt(tx.amount_paid)}</span>
                    </td>
                    <td className="px-5 py-4">
                      <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500 bg-slate-800 px-2 py-0.5 rounded-full">
                        {tx.gateway}
                      </span>
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <code className="text-[11px] text-slate-500 font-mono bg-slate-950 px-2 py-1 rounded-lg border border-slate-800">
