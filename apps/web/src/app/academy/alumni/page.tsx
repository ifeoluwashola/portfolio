...
export default async function AlumniGridPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 selection:bg-yellow-500/30 font-mono pb-32">
      {/* Hero Header */}
      <div className="relative border-b border-slate-900 bg-slate-950/50 backdrop-blur-xl pt-32 pb-20 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          ...
          <p className="text-slate-400 text-sm md:text-lg max-w-2xl leading-relaxed uppercase tracking-wider animate-in fade-in slide-in-from-bottom-8 duration-1000">
      ...
      <main className="max-w-7xl mx-auto px-6 -mt-10 relative z-20">
        {alumni.length === 0 ? (
          <div className="bg-slate-900/40 border border-slate-800 rounded-3xl p-32 text-center">
            ...
            <p className="text-slate-500 font-bold uppercase tracking-widest">Awaiting the first cohort of elites_</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {alumni.map((member: any) => (
              <Link 
                ...
                className="group relative bg-slate-900/30 border border-slate-800 rounded-3xl p-8 hover:border-yellow-500/40 transition-all hover:bg-slate-900/60 overflow-hidden"
              >
                ...
                <div className="flex items-start justify-between mb-8">
                  <div className="w-16 h-16 bg-slate-950 border border-slate-800 rounded-2xl flex items-center justify-center text-yellow-500 text-2xl font-bold group-hover:border-yellow-500/20 transition-all shadow-inner">
                    {member.student_name[0]}
                  </div>
                  <div className="flex gap-3">
                    {member.linkedin_url && <Linkedin className="w-4 h-4 text-slate-600 hover:text-yellow-500 transition-colors" />}
                    {member.github_url && <Github className="w-4 h-4 text-slate-600 hover:text-yellow-500 transition-colors" />}
                  </div>
                </div>

                <div className="space-y-1 mb-10">
                  <h3 className="text-2xl font-bold group-hover:text-yellow-500 transition-colors">{member.student_name}</h3>
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{member.cohort_name}</p>
                </div>
        ...
        {/* Recruitment CTA */}
        <div className="mt-32 border-t border-slate-900 pt-32 grid grid-cols-1 lg:grid-cols-5 gap-12 items-center">
           ...
           <div className="lg:col-span-2 flex flex-col gap-4">
              ...
              <Link href="/academy" className="w-full border border-slate-800 text-slate-400 text-center py-4 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-slate-900 transition-all flex items-center justify-center gap-2">
