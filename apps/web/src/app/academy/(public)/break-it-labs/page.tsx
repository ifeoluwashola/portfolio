...
export default function LabsHubPage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-yellow-500/30 font-mono pb-20">
      <div className="border-b border-border bg-background/80 backdrop-blur-md z-40">
        <div className="max-w-7xl mx-auto px-6 py-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-[10px] text-muted-foreground uppercase tracking-widest mt-1">Communal Debugging &amp; Infrastructure Audits</p>
          </div>
          ...
          <div className="relative max-w-md w-full">
            ...
            <input 
              ...
              className="w-full bg-card/50 border border-border rounded-lg py-2.5 pl-10 pr-4 text-sm text-foreground placeholder:text-muted-foreground/50 focus:ring-1 focus:ring-yellow-500/50 outline-none transition-all"
            />
          </div>
        </div>
      </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        <div className="mb-16 grid grid-cols-1 lg:grid-cols-3 gap-8">
           <div className="lg:col-span-2 bg-yellow-500/5 border border-yellow-500/10 rounded-2xl p-8 relative overflow-hidden group">
              ...
              <div className="relative z-10">
                ...
                <div className="flex flex-wrap gap-4">
                   <div className="flex items-center gap-2 text-[10px] font-bold text-muted-foreground uppercase bg-card/50 px-3 py-1.5 rounded-full border border-border">
           ...
           <div className="bg-card/30 border border-border rounded-2xl p-8 flex flex-col justify-between">
              <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {loading ? (
             Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="h-64 bg-card/40 animate-pulse rounded-2xl border border-border/50" />
             ))
            ...
            filteredLabs.map((lab) => (
              <Link 
                ...
                className="group relative bg-card/20 border border-border rounded-2xl p-6 hover:border-yellow-500/50 transition-all hover:bg-card/40"
              >
                <div className="flex items-start justify-between mb-8">
                  <div className="w-12 h-12 bg-card border border-border rounded-xl flex items-center justify-center text-muted-foreground group-hover:text-yellow-500 group-hover:border-yellow-500/20 transition-all">
                ...
                <p className="text-xs text-muted-foreground leading-relaxed line-clamp-3 mb-8">
                ...
                <div className="flex items-center justify-between pt-4 border-t border-border/50">
                   <span className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-widest">{new Date(lab.created_at).toLocaleDateString()}</span>
            ...
            <div className="col-span-full py-20 text-center border-2 border-dashed border-border rounded-3xl">
               <p className="text-muted-foreground font-bold uppercase tracking-widest">No matching scenarios found_</p>
            </div>
          )}
        </div>
        ...
        {!loading && (
          <div className="mt-32 bg-gradient-to-br from-yellow-500/10 to-card border border-yellow-500/20 rounded-3xl p-12 text-center relative overflow-hidden">
             ...
             <div className="relative z-10">
               ...
               <p className="text-muted-foreground text-[10px] max-w-xl mx-auto mb-10 leading-relaxed uppercase tracking-[0.3em]">
