...
export default function LabDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center font-mono text-yellow-500">_auditing_environment...</div>;
  if (!lab) return <div className="min-h-screen bg-slate-950 flex items-center justify-center text-slate-500 font-mono">FAIL: lab_not_found</div>;

  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 selection:bg-yellow-500/30 font-mono pb-20">
      {/* Detail Header - Removed sticky to avoid clash with AcademyNavbar */}
      <div className="bg-slate-950/80 backdrop-blur-md border-b border-slate-900 z-40">
        <div className="max-w-5xl mx-auto px-6 py-6">
          <Link href="/academy/break-it-labs" className="inline-flex items-center gap-2 text-[10px] font-bold text-slate-500 uppercase tracking-widest hover:text-yellow-500 transition-colors mb-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-4 mt-2">
                 ...
                 <span className={`text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded border ${
                    lab.status === 'active' ? 'text-emerald-500 border-emerald-500/20 bg-emerald-500/5' : 'text-slate-500 border-slate-800'
                 }`}>
      ...
      <div className="max-w-5xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2 space-y-12">
          <section>
            <h2 className="text-sm font-bold uppercase tracking-[0.3em] text-slate-600 mb-6 flex items-center gap-3">
               <ShieldAlert className="w-4 h-4 text-yellow-500" /> Scenario Outline
            </h2>
            <div className="prose prose-invert prose-yellow max-w-none text-slate-400 leading-relaxed text-sm">
               {lab.scenario}
            </div>
          </section>
          ...
          <section>
            ...
            <div className="relative group">
               <div className="absolute top-4 right-4 text-[9px] font-bold text-slate-600 bg-slate-900/50 px-2 py-1 rounded border border-slate-800 uppercase group-hover:text-red-500 transition-colors">Read-Only</div>
               <pre className="bg-slate-900/50 border border-slate-800/50 rounded-2xl p-6 text-xs text-red-100 overflow-x-auto leading-relaxed scrollbar-hide">
          ...
          <section className="pt-12 border-t border-slate-900">
             {isAuthenticated ? (
               <div className="space-y-6">
                 <h2 className="text-sm font-bold uppercase tracking-[0.3em] text-slate-600 flex items-center gap-3">
                    <Terminal className="w-4 h-4 text-yellow-500" /> Proposed Fix_
                 </h2>
                 <p className="text-xs text-slate-500">Existing solutions for this lab will be overwritten (GitHub Force Push Strategy).</p>
                 <textarea 
                   value={newFix}
                 ...
                 {showSuccess && (
                    <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 rounded-xl p-4 text-[10px] font-bold uppercase tracking-widest flex items-center gap-3 animate-in fade-in slide-in-from-top-1">
                ...
                <div className="bg-yellow-500/5 border border-yellow-500/10 rounded-3xl p-10 text-center space-y-6 relative overflow-hidden">
                   ...
                   <p className="text-sm text-slate-400 max-w-sm mx-auto">These technical labs are exclusive to Kybern Academy students. Enrolled members can submit fixes and receive peer code reviews.</p>
                   <Link 
                     href="/academy" 
          ...
          <section className="pt-20 space-y-8">
             ...
             <div className="space-y-12">
               {lab.submissions?.map((sub) => (
                 <div key={sub.id} className={`bg-slate-900/10 border rounded-3xl overflow-hidden transition-all relative ${
                    sub.is_winner ? "border-yellow-500/30" : "border-slate-900"
                 }`}>
                   {sub.is_winner && (
                   ...
                   <div className="p-6 border-b border-slate-900 flex items-center justify-between bg-slate-900/20">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                           <p className="text-sm font-bold text-slate-200">{sub.student_name} <span className="text-slate-600 font-normal">wants to merge fixes</span></p>
                           <p className="text-[10px] text-slate-500 uppercase tracking-widest">SUBMISSION_REF: {sub.id.toString().padStart(4, '0')}</p>
                        </div>
                      </div>
                   </div>
                   ...
                   {/* PR Code */}
                   <div className="p-6">
                      <pre className="bg-slate-950 rounded-2xl p-6 text-[11px] font-mono text-emerald-400 overflow-x-auto border border-slate-900 leading-relaxed">
                         <code>{sub.proposed_fix}</code>
                      </pre>
                   </div>

                   {/* PR Comments Thread */}
                   <div className="bg-black/20 px-6 py-8 border-t border-slate-900 space-y-6">
                      {sub.comments && sub.comments.length > 0 ? (
                        <div className="space-y-6">
                          {sub.comments.map((comment) => (
                            <div key={comment.id} className="flex gap-4">
                               <div className="w-6 h-6 rounded bg-slate-800 flex-shrink-0 flex items-center justify-center text-[10px] font-bold">
                                  {comment.student_name[0]}
                               </div>
                               <div className="flex-1 space-y-1">
                                  <div className="flex items-center justify-between">
                                     <span className="text-[10px] font-bold text-yellow-500/80 uppercase tracking-widest">{comment.student_name}</span>
                                     <span className="text-[9px] text-slate-600">{new Date(comment.created_at).toLocaleDateString()}</span>
                                  </div>
                                  <div className="p-3 bg-slate-900/50 rounded-xl border border-slate-800/50 text-xs text-slate-400 leading-relaxed">
                                     {comment.body}
                                  </div>
                               </div>
                            </div>
                        ...
                        <p className="text-[10px] text-slate-600 italic uppercase tracking-widest text-center py-4">No reviews yet_</p>
                      )}
                      ...
                      {isAuthenticated ? (
                         <div className="pt-6 border-t border-slate-900/50">
                            {commentingOn === sub.id ? (
                               <div className="space-y-3 animate-in fade-in slide-in-from-top-2">
                                  ...
                                  <div className="flex justify-end gap-3">
                                     <button 
                                       ...
                                       className="px-4 py-2 text-[9px] font-bold text-slate-600 uppercase hover:text-slate-400"
                                     >Cancel</button>
                                     <button 
                               ...
                               <button 
                                 ...
                                 className="w-full bg-slate-950/50 border border-slate-900 rounded-xl py-3 text-[10px] font-bold text-slate-500 hover:text-yellow-500 transition-all uppercase tracking-widest flex items-center justify-center gap-2"
                               >
                        ...
                        <div className="pt-6 border-t border-slate-900/50 text-center">
                           <Link href="/academy/login" className="text-[10px] font-bold text-slate-500 hover:text-yellow-500 transition-colors uppercase tracking-widest flex items-center justify-center gap-2">
        ...
        {/* Right Column: Lab Meta & CTA */}
        <div className="space-y-8">
           <div className="bg-slate-900/30 border border-slate-800 rounded-3xl p-8 sticky top-32">
              ...
              <ul className="space-y-6">
                 <li className="flex gap-4">
                    <div className="w-5 h-5 bg-yellow-500/10 rounded flex items-center justify-center text-[10px] font-bold text-yellow-500 flex-shrink-0">1</div>
                    <p className="text-[11px] text-slate-500 leading-relaxed uppercase tracking-tighter">Only one active fix permitted per student. Force push strategy is active.</p>
                 </li>
                 <li className="flex gap-4">
                    <div className="w-5 h-5 bg-yellow-500/10 rounded flex items-center justify-center text-[10px] font-bold text-yellow-500 flex-shrink-0">2</div>
                    <p className="text-[11px] text-slate-500 leading-relaxed uppercase tracking-tighter">Verified winners will be immortalized in the lab history as Lead Architects.</p>
                 </li>
                 <li className="flex gap-4">
                    <div className="w-5 h-5 bg-yellow-500/10 rounded flex items-center justify-center text-[10px] font-bold text-yellow-500 flex-shrink-0">3</div>
                    <p className="text-[11px] text-slate-500 leading-relaxed uppercase tracking-tighter">Spamming review threads will result in academic suspension.</p>
                 </li>
              </ul>
              <div className="mt-12 pt-8 border-t border-slate-800">
                 <p className="text-[10px] font-bold text-slate-600 uppercase mb-4 tracking-widest">Share Scenario</p>
                 <div className="flex gap-4">
                    <button className="text-slate-400 hover:text-yellow-500 transition-colors"><Terminal className="w-4 h-4" /></button>
                    <button className="text-slate-400 hover:text-yellow-500 transition-colors"><Zap className="w-4 h-4" /></button>
                 </div>
              </div>
           </div>
        </div>
      ...
      {!isAuthenticated && (
        <div className="max-w-5xl mx-auto px-6 mt-32">
          <div className="bg-gradient-to-br from-yellow-500/10 to-slate-900 border border-yellow-500/20 rounded-3xl p-12 text-center relative overflow-hidden">
             <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-10 pointer-events-none" />
             <div className="relative z-10">
               <h2 className="text-3xl font-bold mb-4 tracking-tighter uppercase">Enjoyed this <span className="text-yellow-500">scenario?</span></h2>
               <p className="text-slate-400 text-[10px] max-w-sm mx-auto mb-10 leading-relaxed uppercase tracking-[0.3em]">
