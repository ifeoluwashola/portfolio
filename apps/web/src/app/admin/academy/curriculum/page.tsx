...
export default function CurriculumManager() {
  ...
  async function fetchWeeks() {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8081/api"}/v1/admin/academy/weeks`, {
        headers: {
  ...
  if (loading) return <div className="p-8 animate-pulse text-muted-foreground font-mono">Initializing curriculum terminal...</div>;
    ...
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      {editingWeek && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
          <div className="bg-card w-full max-w-lg border border-border rounded-2xl shadow-2xl overflow-hidden">
            <div className="p-6 border-b border-border bg-muted/30">
              <h2 className="text-xl font-bold">Week {editingWeek.week_number}: {editingWeek.title}</h2>
              <p className="text-sm text-muted-foreground mt-1">Configure live environment and metadata.</p>
            </div>
            ...
            <form onSubmit={handleUpdateWeek} className="p-6 space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold">Live Meeting URL (GMeet/Zoom)</label>
                <input 
                  type="url"
                  className="w-full p-2 bg-background border border-border rounded-md"
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold">Recording Link (Google Drive/YouTube)</label>
                <input 
                  type="url"
                  className="w-full p-2 bg-background border border-border rounded-md"
              ...
              <div className="flex gap-3 pt-2">
                ...
                <Button 
                  ...
                  disabled={isUpdating}
                >
                  {isUpdating ? "Applying Changes..." : "Commit Deployment"}
                </Button>
              </div>
            </form>
          </div>
